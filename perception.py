...
def obstacle_thresh(img, rgb_thresh=(160, 160, 160)):
    ...
    below_thresh = (img[:,:,0] <= rgb_thresh[0]) \
                ...
                | (img[:,:,2] <= rgb_thresh[2])
    # Index the array of zeros with the boolean array and set to 1
    #mask_invert=np.invert(rock_image)
    color_select[below_thresh] = 255
    #mask_obstacles_or_rock =cv2.bitwise_and(color_select,mask_invert,mask=None)
    # Return the binary image
    return color_select
...
def perception_step(Rover):
    ...
    # if rock found
    if(rock_angles.any()):
        # let the nav angles and distance of navigable terrain to that of the rock
        angle = rock_angles
        dist = rock_dist
        Rover.nav_angles_when_rock=ground_angle
        Rover.nav_dist_when_rock=ground_dist
        is_rock_nav_direction=1
        is_ground_nav_direction=0
        Rover.mode = 'rock'
    else:
        angle = ground_angle
        dist = ground_dist
        is_ground_nav_direction=1
        is_rock_nav_direction=0
        # if rock no longer near set mode to forward
        if Rover.mode =='rock':
            Rover.mode = 'forward'
